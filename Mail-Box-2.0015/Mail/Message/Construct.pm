...
The general methods for C<Mail::Message::Construct> objects:
      ...
      build [MESSAGE|BODY], CONTENT        quotePrelude [STRING|FIELD]
...
capital are used as header-lines.  Next to header-lines, the message body
has to be specified with fields which start with lower-case characters.
Each field may be used more than once.
...
Example:
 ...
 my $msg = Mail::Message->buildFromBody($body, From => 'me@example.nl',
    To => 'you@example.com');
...
{   my ($class, $body) = (shift, shift);
    ...
    {   use POSIX 'strftime';
        $head->set(Date => strftime "%a, %d %b %Y %H:%M:%S %z", localtime);
    }
