...
with creating a body, and transform that into a message using
C<Mail::Message::buildFromBody>, or by creating the whole message at once
with C<Mail::Message::build>.  Both are described in
...
you when executing the next line.
 ...
 my $message = Mail::Message->buildFromBody($body
   , From => 'me@example.com', To => 'you@anywhere.net');

If you want to create multi-part messages, you need to create a multi-part
body yourself first.
