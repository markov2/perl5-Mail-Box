...
For example:
 ...
 my $body    = Mail::Message::Body::Scalar->new(mime_type => 'image/gif');
...
The general methods for C<Mail::Message::Body> objects:
 ...
 MMBC attach MESSAGES                      message [MESSAGE]
 ...
 MMBE encode OPTIONS                       reply OPTIONS
   MR errors                            MR report [LEVEL]
      file                              MR reportAll [LEVEL]
 MMBC foreachLine CODE                     size
      isBinary                             string
      isDelayed                       MMBC stripSignature OPTIONS
      isMultipart                       MR trace [LEVEL]
      lines                                transferEncoding
   MR log [LEVEL [,STRINGS]]               type
...
=item * data =E<gt> FILEHANDLE | ARRAY-OF-LINES | STRING
...
Specify a FILEHANDLE, which refers to a opened file which allows reading,
a reference to an ARRAY of lines, or one STRING which may contain
multiple lines.
...
field is case-insensitive.  The default mime-type is C<text/plain>,
...
Examples:
 ...
 my $body = Mail::Message::Body::String->new(data => \*IN,
...
{   my ($self, $args) = @_;
    ...
    my $type = defined $data ? ref $data : undef;
    ...
    {   my $file = FileHandle->new($filename, 'r');
        ...
        $self->_data_from_file($file);
        $file->close;
    }
    elsif(!defined $type) {;}
    elsif(!$type)
    {   # one scalar with lines.
        my $data = [ split /(?<=\n)/, $data ];
        $self->_data_from_lines($data);
    }
    elsif($type eq 'GLOB' || $type->isa('IO::Handle'))
    {   # a filehandle
        $self->_data_from_file($data);
    }
    elsif($type eq 'ARRAY')
    {   # array of lines.
        $self->_data_from_lines($data);
    ...
    {   # huh?
        $self->log(ERROR =>
            "You offered me a $type for data: cannot handle that.");
        return;
    }
    ...
    if(defined $based)
    {   $mime     = delete $args->{mime_type}   || $based->type;
        $transfer = delete $args->{transfer_encoding}||$based->transferEncoding;
        $disp     = delete $args->{disposition} || $based->disposition;
        $self->{MMB_checked} = delete $args->{checked} || $based->checked;
    }
    else
    {   $mime     = delete $args->{mime_type}         || 'text/plain';
        $transfer = delete $args->{transfer_encoding} || 'none';
        $disp     = delete $args->{disposition}       || 'none';
        $self->{MMB_checked} = delete $args->{checked}|| 0;
    }

    $mime = Mail::Message::Field->new('Content-Type' => lc $mime)
        unless ref $mime;

    $mime->attribute(charset => delete $args->{charset} || 'us-ascii')
        if $mime->body =~ m!^text/!;
...
sub transferEncoding() { shift->{MMB_transfer}  }
...
Returns whether the body is in un-encoded binary format or in printable
text format.  All un-encoded mime-types except in classes C<text/> and
C<multipart/> are considered binary formats.

Bodies with binary content can not be transmitted, nor written to file;
if a binary body is added to a message-object it will automatically
get encoded.
...
sub isBinary()
{   my $self = shift;
    return 0 unless $self->{MMB_transfer};

    $self->{MMB_type}->body !~ m!^(text|multipart)/!;
}
