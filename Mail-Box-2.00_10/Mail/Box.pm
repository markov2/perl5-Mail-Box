...
where FOLDER is a reference to the folder we are reading.  HEAD refers to an
C<Mail::Message::Head>.  The routine must return a true value (be lazy: delay
...
Examples:
    ...
    $folder->new(lazy_extract => sub
       { my ($f, $head) = @_;
         my $size = $head->guessBodySize;
         defined $size ? $size >= 10000 : 0
       }); #same
...
#       weaken $self->{MB_manager};
    ...
    my $headtype = delete $args->{MB_head_type};
    confess "head_type must be complete, but is $headtype.\n"
        if $headtype && !$headtype->isa('Mail::Message::Head::Complete');

    $self->{MB_head_type} = $headtype || 'Mail::Message::Head::Complete';
    my $extract           = $args->{lazy_extract} || 10000;
    $self->{MB_lazy_extract}
      = ref $extract eq 'CODE' ? $extract
      : $extract eq 'NEVER'    ? sub {0}
...
if($self->{MB_modified}) {
warn "Modified $self->{MB_modified}\n";
...
{   my ($self, $message, $head) = @_;
    ...
    {   my $delayed = $self->{MB_body_delayed_type};
        my $extract = $self->{MB_lazy_extract};
        return $delayed if ref $extract && $extract->($self, $head);
