...
The general methods for C<Mail::Message::Head> objects:
      ...
      add ...                              new OPTIONS
      count NAME                           nrLines
   MR errors                               print FILE [,LINE-LENGTH]
      get NAME [,INDEX]                 MR report [LEVEL]
      isDelayed                         MR reportAll [LEVEL]
      isMultipart                          reset NAME, FIELDS
      knownNames                           set ...
   MR log [LEVEL [,STRINGS]]               size
      modified [BOOL]                      timestamp
      names                             MR trace [LEVEL]

The extra methods for extension writers:

   MR DESTROY                           MR inGlobalDestruction
      addNoRealize FIELD                   load
      clone [FIELDS]                    MR logPriority LEVEL
      createFromLine                    MR logSettings
      createMessageId                      message [MESSAGE]
      grepNames [NAMES|ARRAY-OF-N...    MR notImplemented
      guessBodySize                        read PARSER
      guessTimestamp                       setNoRealize FIELD
