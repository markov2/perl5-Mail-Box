...
Create a filter.
 ...
 include_delayed  Mail::Box::Search  1
 include_deleted  Mail::Box::Search  0
...
=item * details =E<gt> undef|'PRINT'|REF-ARRAY|CODE
...
found.  You may also specify your own CODE reference.  With an reference
...
{   my ($self, $args) = @_;
    ...
    $self->{MBS_no_deleted} = not $args->{include_deleted};
    $self->{MBS_no_delayed} = not $args->{include_delayed};
    $self->{MBS_multiparts}
...
{   my ($self, $object) = @_;
    ...
    @messages = reverse @messages if $limit < 0;
...
{  my ($self, $part) = @_;
   ...
   {   return $matched unless $self->{MBS_multiparts};
       my $no_delayed = not $self->{MBS_include_delayed};
...
This code is beta, version 2.008.
