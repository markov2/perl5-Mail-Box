...
The general methods for C<Mail::Message> objects:
   ...
   MR errors                               parent
      ...
      isMultipart                          size
      isPart                               timestamp
   MR log [LEVEL [,STRINGS]]               toplevel
      messageId                         MR trace [LEVEL]

The extra methods for extension writers:

      DESTROY                           MR logPriority LEVEL
      body [BODY, OPTIONS]              MR logSettings
      clone                             MR notImplemented
      coerce MESSAGE [,OPTIONS]            read PARSER, [BODYTYPE]
...
sub bodyhandle()
{   my $self = shift;
    $self->isMultipart ? undef : $self->decoded;
}
...
{   my $self = shift;
      ...
      ! $self->isMultipart ? ()
    : ! @_                 ? map {$_->decoded} $self->body->parts
    :                        $self->body->part(shift)->decoded;
}
...
file, and the HEAD is already read.  Information from the HEAD may be used
to create expectations about the message's length.  The BODYTYPE determines
which kind of body will be made and defaults to the value specified by the
C<body_type> option at message creation (see C<new>).

BODYTYPE may be the name of a body class, or a reference to a routine
...
{   my ($self, $parser, $head, $getbodytype) = @_;
    ...
    $bodytype->new
      ( message => $self
      , $self->logSettings
      )->read($parser, $head, $getbodytype, $lines, $size);
}
...
want to update the related header fields too.  When C<undef> is
explicitly specified, the BODY is removed (and the old returned)
...
{   my $self = shift;
    ...
    my ($body, %args) = @_;
    return delete $self->{MM_body} unless $body;

    $self->log(INTERNAL => "wrong type of body for $body")
        unless ref $body && $body->isa('Mail::Message::Body');

    $body->message($self);
    $body->modified(1) if $self->{MM_body};
