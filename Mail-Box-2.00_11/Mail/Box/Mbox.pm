...
The general methods for C<Mail::Box::Mbox> objects:
   ...
   MB activeMessage INDEX [,MESSAGE]    MB locker
   ...
   MB allMessages                       MB messages
   MB close OPTIONS                     MB modified
...
The extra methods for extension writers:
   ...
   MB folderdir [DIR]                   MB storeMessage MESSAGE
      foundIn FOLDERNAME [,OPTIONS]     MB timespan2seconds TIME
...
{   my $self = shift;
    ...
    while(1)
    {   my $message = $self->{MB_message_type}->new(@msgopts);
