...
The general methods for C<Mail::Box::Parser> objects:
      ...
      bodyAsString [,CHARS [,LINES]]       readHeader
      defaultParserType [PARSER]               readSeparator OPTIONS
   MR errors                            MR report [LEVEL]
      foldHeaderLine LINE, LENGTH       MR reportAll [LEVEL]
      inDosmode                            setPosition WHERE
   MR log [LEVEL [,STRINGS]]               start OPTIONS
      new [OPTIONS]                        stop
      popSeparator                      MR trace [LEVEL]
      pushSeparator STRING              MR warnings
...
Set the read-location in the folder to the specified value.  Both header
and body-objects contain there location, which can be retreived with their
C<start()> methods.

=cut

sub setPosition($) {shift->NotImplemented}
