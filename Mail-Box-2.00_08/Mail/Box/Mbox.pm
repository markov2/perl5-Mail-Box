...
The general methods for C<Mail::Box::Mbox> objects:
   ...
   MB DESTROY                              listFolders [OPTIONS]
   ...
   MB find MESSAGE-ID                   MR reportAll [LEVEL]
      folderToFilename FOLDERNAME...    MR trace [LEVEL]
   MB folderdir [DIR]                   MR warnings
      foundIn FOLDERNAME [,OPTIONS]        write

The extra methods for extension writers:

   MB coerce MESSAGE                    MB organization
   MB lazyExtract HEADER, BODY, SIZE    MB readBodyType HEAD, SIZE [,L...
   MR logPriority LEVEL                 MB scanForMessages MESSAGE, ME...
   MR logSettings                       MB sort PREPARE, COMPARE, LIST
   MB messageDeleted MESSAGE|MESS...    MB timespan2seconds TIME
   MR notImplemented                    MB toBeThreaded MESSAGES
...
description of Mbox specific options.
 ...
 lock_file           Mail::Box::Locker  foldername.lock-extension
...
#warn "Reading $filename.\n";
    ...
    while(1)
    {   my $message = $msgtype->new($self->logSettings);
#warn "next message.\n";
        last unless $message->read($parser, $headtype, $bodytype, $wrap);
#warn "message read.\n";

        $delayed++ if $message->isDelayed;
