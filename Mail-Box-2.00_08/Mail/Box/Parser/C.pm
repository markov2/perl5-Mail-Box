...
sub setPosition($)
{   my $self = shift;
    set_position $self->{MBPC_file}, shift;
}

sub readHeader()   {read_header shift->{MBPC_file} }
...
static void logmsg(Mailbox *box, SV *level, SV *text);
...
static int read_header_line(Mailbox *box, SV **field, SV **content)
{
    ...
    field_error = 0;
    for(length = reader-line; length >= 0 && isspace(line[length]); --length)
        field_error++;

    if(field_error && box->trace <= TRACE_WARNINGS)
    {   logmsg(box, warning,
               newSVpvf("Blanks stripped after header-fieldname:\n  %s",line));
    }
    ...
    *field = newSVpvn(line, length);
    ...
    for(; *reader!='\n'; ++reader)
        ;
    while(reader >= line && isspace(*reader))
        *reader-- = EOL;

    if(reader < line && box->trace <= TRACE_NOTICES)
        logmsg(box,notice, newSVpvf( "Empty header content for field %s"
                      , SvPV_nolen(*field)));
   
    *content = newSVpv(line, 0);
...
static void print_SV(SV *sv, FILE *out)
{   STRLEN   length;
    char   * string;
fprintf(out, "xxx.\n");
...
{
return;
    print_SV(level, stderr);
    fputs(": ", stderr);
    print_SV(text, stderr);
    putc('\n', stderr);

/* TBI
    call_perl_box->logmsg(SvREFCNT_inc(level),
