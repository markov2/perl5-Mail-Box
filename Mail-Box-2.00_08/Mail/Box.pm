...
The general methods for C<Mail::Box> objects:
      ...
      DESTROY                              listFolders OPTIONS
      activeMessage INDEX [,MESSAGE]    MR log [LEVEL [,STRINGS]]
      addMessage  MESSAGE                  message INDEX [,MESSAGE]
      addMessages MESSAGE [, MESS...       messageId MESSAGE-ID [,MESS...
      allMessageIds                        messages
      allMessages                          modifications INCR
      appendMessages OPTIONS               modified
      clone OPTIONS                        name
      close OPTIONS                        new OPTIONS
      create FOLDERNAME [, OPTIONS]        read OPTIONS
      current [NUMBER|MESSAGE|MES...    MR report [LEVEL]
      delete                            MR reportAll [LEVEL]
   MR errors                            MR trace [LEVEL]
      find MESSAGE-ID                   MR warnings
      folderdir [DIR]                      write OPTIONS
      foundIn FOLDERNAME, OPTIONS          writeable

The extra methods for extension writers:

      coerce MESSAGE                       organization
      lazyExtract HEADER, BODY, SIZE       readBodyType HEAD, SIZE [,L...
   MR logPriority LEVEL                    scanForMessages MESSAGE, ME...
   MR logSettings                          sort PREPARE, COMPARE, LIST
      messageDeleted MESSAGE|MESS...       timespan2seconds TIME
   MR notImplemented                       toBeThreaded MESSAGES
...
you will not specify these:
 ...
 head_partial_type Mail::Box          'Mail::Message::Head::Partial'
...
the next three option specify the algorithm. C<Lazy_extract> determines whether
...
The second argument is the expected size of the message (or C<undef>)

Do I<not> return a delayed body-type (like C<::Delayed>), because that is
determined by the C<lazy_extract()> method.  Do always check for multipart
...
{   my ($self, $args) = @_;
    ...
    $args->{MB_head_wrap}    = $args->{head_wrap} if defined $args->{head_wrap};
...
#       weaken $self->{MB_manager};
    ...
    $self->{MB_head_partial_type}
        = $args->{head_partial_type}|| 'Mail::Message::Head::Partial';
...
sub END {$global_destruction++}

sub DESTROY
{   my $self = shift;
    $self->close unless $global_destruction || $self->{MB_is_closed};
}
...
{   my ($self, $message) = @_;
      ...
      $message->isa($self->{MB_message_type})
    ? $message
    : $self->{MB_message_type}->coerce($self, $message);
}
...
=item readBodyType HEAD, SIZE [,LAZY]

Determine which kind of body will be created for this message when
reading the folder initially.  This method uses the HEAD to check for
the type and a SIZE (may be C<undef>).

If LAZY is set true, then the C<lazyExtract> is consulted.  Otherwise,
no delay loading will take place.

=cut

sub readBodyType($$;$)
{   my ($self, $head, $size, $lazy) = @_;

      $lazy && $self->lazyExtract($head, undef, $size)
                                ? $self->{MB_body_delayed_type}
    : $head->isMultipart        ? $self->{MB_multipart_type}
