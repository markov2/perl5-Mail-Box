...
C<Mail::Box> suite you have to read C<Mail::Box-Overview>.  But you
do not need to know much if you want to use the C<Mail::Box> suite.
...
L<Mail::Message::Construct>.  For a single-part reply, the return
will be
...
=item * C<Mail::Message-E<gt>buildFromBody>

Start with creating a body, and transform that into a message.

=item * C<Mail::Message-E<gt>build>

create the whole message at once.

=item * C<Mail::Message-E<gt>read>

read a message from a file-handle, scalar, or array of lines.

=back

All three methods are implemented in L<Mail::Message::Construct>.  Please,
do yourself a favour, and give preference to the C<build*> methods, over
...
For instance, C<$message->decoded> returns the decoded body of the
...
it is added to a message.  The default encoding dependends on the mime type.
