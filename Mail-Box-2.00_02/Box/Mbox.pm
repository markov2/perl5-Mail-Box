...
This manual-page describes how Mbox-kind of mail-boxes work, and in
detail what you can do with the Mbox folder handler object C<Mail::Box::Mbox>.
...
{   my $self = shift;
    ...
    while(my $head = $headtype->new->read($file))
    {
        my $size = $head->guessBodySize;
        my $bodytype;

        if($self->lazyExtract($head, undef, $size))
        {   $bodytype = 'Mail::Message::Body::Delayed';
            $delayed++;
        }
