...
Mail::Box::Thread - one node in a Mail::Box::Threads data structure
...
# DWC I hope I got that last sentence right.
The C<Mail::Box::Thread> maintains one node in the linked list of threads.
...
You will not call this method yourself. The C<Mail::Box::Threads> object
will use it to construct C<Mail::Box::Thread> objects.
...
placeholders in a C<Mail::Box::Threads> data structure.
...
message (see C<Mail::Message::Dummy>).
