...
provide an object-oriented back-end for Mail User-Agent (MUA) programs.
...
body of a message.  There are various header-objects and a bunch of
body objects.
...
One of the body-types is the C<Mail::Message::Body::Multipart>, which
...
The (complete) header for a single message. Maintains a set of
C<Mail::Message::Field> objects, each containing one header line.
Two sub-classes for a header contain only a sub-set of all lines,
but will get the rest for you when needed.

=item C<Mail::Box::Threads>
...
This object maintains linked lists of C<Mail::Message::Thread> objects.
C<Mail::Message::Dummy> fill-up some holes.

=item C<Mail::Box::Thread>

A single node in a thread.  This contains a reference to a node which
contains the message where this message was a reply to (or C<undef>)
and a list of nodes which contain the reply message to this one.

=item C<Mail::Box::Index>

A base class for mailbox indexes which provides methods for reading,
writing, and managing indexes.  These indexes are used by some
kinds of folders to speed-up access.
...
filehandle locking, and locking over NFS.
...
Provides hash or array tied access to any mail folder derived from
C<Mail::Box>.  This beautifies your code.
