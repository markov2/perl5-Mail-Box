...
C<'NOTICES'> C<'DEBUG'>, and C<'NONE'>.  The C<PROGRESS> is about reading
and writing folders.  C<NONE> only will let C<INTERNAL> errors pass.

By the way: C<ERROR> is an alias for C<ERRORS>,
as C<WARNING> is an alias for C<WARNINGS>, and C<NOTICE> for C<NOTICES>.

When you specify C<NONE>,
...
sub init($)
{   my ($self, $args) = @_;
    $self->{MR_log}   = $self->log  ($args->{log}   || 'WARNING');
    $self->{MR_trace} = $self->trace($args->{trace} || 'WARNING');
    $self;
}
