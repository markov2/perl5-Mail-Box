...
Tied-interface:
   ...
   # $inbox is a Mail::Box (sub-class?) now.
...
The options added by C<Mail::Box>:
...
Which class is the base-class for maintaining message-body objects.  By
default, this will be C<Mail::Message::Body>.
...
Of which type are full parsed message-headers.  By default, this will be
C<Mail::Message::Head>.

=item * head_fold =E<gt> INTEGER

Fold the structured headers on the specified length (defaults to C<72>).
Folding is disabled when C<0> is specified.

=item * head_partial_type =E<gt> CLASS

The type of partial headers: some lines do exist, but maybe not all.  There
are to situtations when these headers may appear: when you specify
only specific elements at C<take_header> (see discussion below) or when
the folder-type has a way to get a fast but abbreviated overview of the
headers.  This latter is the case for C<emh> and C<IMAP> folders.
...
Mail::Box::Mbox.  Best use the folder-manager (see Mail::Box::Manager).
USAGE
    }
...
to be read into a C<Mail::Message::Head>-object (to be more precise: the
...
sub registeredHeaders() { keys %{shift->{MB_take_headers}} }
