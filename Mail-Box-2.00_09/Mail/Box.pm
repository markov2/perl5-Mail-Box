...
The general methods for C<Mail::Box> objects:
   ...
   MR errors                            MR reportAll [LEVEL]
      ...
      listFolders OPTIONS               MR warnings

The extra methods for extension writers:

      DESTROY                              organization
      appendMessages OPTIONS               read OPTIONS
      clone OPTIONS                        readBodyType HEAD, SIZE [,L...
      coerce MESSAGE                       readMessages
      folderdir [DIR]                      scanForMessages MESSAGE, ME...
      foundIn FOLDERNAME, OPTIONS          sort PREPARE, COMPARE, LIST
      lazyExtract HEADER, BODY, SIZE       timespan2seconds TIME
   MR logPriority LEVEL                    toBeThreaded MESSAGES
   MR logSettings                          toBeUnthreaded MESSAGES
   MR notImplemented                       write OPTIONS
...
sub messages()
{   my $self = shift;
    grep {not $_->deleted} $self->allMessages;
}

sub as_row() { [ shift->messages ] }
...
ERROR
    ...
    push @{$self->{MB_messages}}, $coerced;

    $coerced->seqnr( @{$self->{MB_messages}} -1);
    $self->{MB_modified}++;
    $coerced;
}
...
{   my ($self, %args) = @_;
    ...
    $args{messages}
        = $args{save_deleted} ? $self->{MB_messages} : [ $self->messages ];
