...
The general methods for C<Mail::Box::Message> objects:
   ...
   MM attach MESSAGES [,OPTIONS]        MR log [LEVEL [,STRINGS]]
      copyTo FOLDER                     MM messageId
   MM decoded OPTIONS                   MM modified [BOOL]
      delete                               new OPTIONS
      deleted [BOOL]                    MM nrLines
...
{   my $self   = shift;
    ...
    if(my $status = $self->head->get('status'))
    {   $labels->{seen} = $status =~ /R/;
        $labels->{old}  = $status =~ /O/;
    }
    if(my $xstatus = $self->head->get('x-status'))
...
{   my $self = shift;
    my ($seen, $old, $replied, $flagged)
       = $self->labels( qw/seen old replied flagged/ );
