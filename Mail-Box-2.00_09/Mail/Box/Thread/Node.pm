...
=head1 NAME
 ...
 Mail::Box::Thread::Node - one node in a Mail::Box::Thread::Manager data structure
...
The general methods for C<Mail::Box::Thread::Node> objects:
      ...
      expand [BOOL]                        new OPTIONS
      followUps                            numberOfMessages
      followedBy THREADS                   recurse CODE-REF
      follows THREAD, QUALITY              repliedTo
      ids                                  sortedFollowUps [PREPARE [,...
      isDummy                              startTimeEstimate
...
{   my ($self, $code) = @_;
    ...
    $_->recurse($code) or last foreach $self->followUps;
    $self;
}
...
{   my $self  = shift;
    my $total = 0;
    $self->recurse( sub {$total += shift->{MBTN_messages}[0]->size; 1} );
    $total;
}
