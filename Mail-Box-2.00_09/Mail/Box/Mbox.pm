...
The extra methods for extension writers:
   ...
   MB coerce MESSAGE                    MB readBodyType HEAD, SIZE [,L...
      folderToFilename FOLDERNAME...    MB readMessages
   MB folderdir [DIR]                   MB scanForMessages MESSAGE, ME...
      foundIn FOLDERNAME [,OPTIONS]     MB sort PREPARE, COMPARE, LIST
   MB lazyExtract HEADER, BODY, SIZE    MB timespan2seconds TIME
   MR logPriority LEVEL                 MB toBeThreaded MESSAGES
   MR logSettings                       MB toBeUnthreaded MESSAGES
...
{   my ($self, $args) = @_;
    ...
    {   unlink $filename or $self->log(WARNING
    ...
    unless($new)
    {   $self->log(ERROR
           => "Unable to write to file $tmpnew for folder $self: $!");
        return;
    }
    ...
    move $tmpnew, $filename or $self->log(WARNING
