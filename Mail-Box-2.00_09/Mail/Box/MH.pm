...
The general methods for C<Mail::Box::MH> objects:
   ...
   MB activeMessage INDEX [,MESSAGE]    MR log [LEVEL [,STRINGS]]
   ...
   MB allMessages                       MB modified
   MB close OPTIONS                     MB name
   MB create FOLDERNAME [, OPTIONS]        new OPTIONS
   MB current [NUMBER|MESSAGE|MES...    MB openSubFolder NAME [,OPTIONS]
   MB delete                            MR report [LEVEL]
      directory                         MR reportAll [LEVEL]
   MR errors                            MR trace [LEVEL]
   MB find MESSAGE-ID                   MR warnings
...
The extra methods for extension writers:
   ...
   MB coerce MESSAGE                       readAllHeaders
      folderToDirectory FOLDERNAM...    MB readBodyType HEAD, SIZE [,L...
   MB folderdir [DIR]                      readMessageFilenames DIRECTORY
   MB foundIn FOLDERNAME, OPTIONS       MB readMessages
      highestMessageNumber              MB scanForMessages MESSAGE, ME...
      index                             MB sort PREPARE, COMPARE, LIST
      labels                            MB timespan2seconds TIME
   MB lazyExtract HEADER, BODY, SIZE    MB toBeThreaded MESSAGES
...
folders you will not specify these:
 ...
 head_partial_type Mail::Box          <not used>
...
{   my ($self, $msgid) = (shift, shift);
    ...
    foreach my $message (reverse $self->allMessages)
    {   $message->head;
...
sub allMessageIDs() {shift->allMessageIDs}  # compatibility
...
{   my $self = shift;
    ...
    my $bodytype = sub {$self->readBodyType(@_, 1)};
    my $wrap     = $self->{MB_head_wrap};

    foreach my $msgnr (@msgnrs)
    {
        my $msgfile = File::Spec->catfile($directory, $msgnr);
        my $head    = $index  ? $index->get($msgfile) : undef;
...
{   my ($self, $msgnr, $force_read_all) = @_;
    ...
    {   # Create a real header structure, but not yet the body.
        $message->{MBM_head} = MIME::Head->new(\@header);
    }
...
{   my ($self, $args) = @_;
    ...
    $self->labels->write(@messages);
    $self->index->write(@messages);
...
then C<lazy_extract> to determine how far this
