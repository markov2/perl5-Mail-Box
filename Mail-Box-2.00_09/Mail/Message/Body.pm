...
The general methods for C<Mail::Message::Body> objects:
      ...
      data FILE | LIST-OF-LINES |...       nrLines
   MR errors                               print [FILE]
      file                                 reply OPTIONS
      isDelayed                         MR report [LEVEL]
      isMultipart                       MR reportAll [LEVEL]
      lines                                size
   MR log [LEVEL [,STRINGS]]               string
      message [MESSAGE]                    stripSignature OPTIONS
...
The extra methods for extension writers:
   ...
   MR logPriority LEVEL                    read PARSER, HEAD, BODYTYPE...
...
sub data(@)
{   my $self = shift;
    $self->{MMB_modified}++ if $self->{MMB_has_data}++;
