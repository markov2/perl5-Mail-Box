...
Read C<Mail::Box::Manager> and C<Mail::Box> first.  You need to understand
the C<Mail::Box::Thread> too.
...
Type of the threads, by default C<Mail::Box::Thread>
...
derived from C<Mail::Box::Threads>.
...
{   my ($self, $args) = @_;
    ...
    $self->{MBT_dummy_type}  = $args->{dummy_type}  || 'Mail::Message::Dummy';
...
{   my $self = shift;
    ...
    {   croak "Not a folder: $folder"
        ...
        {   $self->inThread($_) unless $_->headIsDelayed;
        }
    }
