...
If there is only one element defined for this field, you will get that
element.  However, if this field was found more than one in the header,
you get all values (in order as read) in LIST-context and the last value
in SCALAR-context.
...
A mailbox maintains many messages, and each message has many many
header-lines.  The C<Mail::Message::Head>-object maintains the
header-lines (I<fields>) for one message.

Some fields are mentioned more than once in one header.  In this case,
we maintain an (ordered on arrival) array of them.  Fields which have
been seen only once are stored as single element.

The order is important when the message gets printed.  The print order
is the same as that the fields where added to the header.
