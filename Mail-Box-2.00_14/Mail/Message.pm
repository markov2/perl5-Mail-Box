...
The general methods for C<Mail::Message> objects:
  ...
  MMC buildFromBody BODY, HEADERS          nrLines
      decoded OPTIONS                      parent
      encode OPTIONS                       print [FILEHANDLE]
   MR errors                           MMC quotePrelude [STRING|FIELD]
      get FIELD                        MMC reply OPTIONS
      guessTimestamp                   MMC replySubject STRING
      isDummy                           MR report [LEVEL]
      isMultipart                       MR reportAll [LEVEL]
      isPart                               size
