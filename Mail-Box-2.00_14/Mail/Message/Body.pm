...
{   my ($self, $args) = @_;
    ...
    if(defined $filename)
    {   $self->_data_from_filename($filename) or return;
    }
    elsif(!defined $type) {;}
    elsif(!$type)
    ...
    {    $self->_data_from_glob($data) or return;
    }
    elsif($type->isa('IO::Handle'))
    {    $self->_data_from_filehandle($data) or return;
    }
    elsif($type eq 'ARRAY')
    ...
    if(defined $based)
    {   $mime     = $args->{mime_type}         || $based->type;
        $transfer = $args->{transfer_encoding} || $based->transferEncoding;
        $disp     = $args->{disposition}       || $based->disposition;
        $self->{MMB_checked} = defined $args->{checked}
           ? $args->{checked} : $based->checked;
    }
    else
    {   $mime
          = defined $args->{mime_type} ? $args->{mime_type}
          : $filename                  ? $mime_types->extension($filename)
          :                              'text/plain';

        $transfer = $args->{transfer_encoding} || 'none';
        $disp     = $args->{disposition}       || 'none';
