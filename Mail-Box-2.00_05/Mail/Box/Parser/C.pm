...
The general methods for C<Mail::Box::Parser::C> objects:
  ...
  MBP defaultParser [PARSER]           MBP readHeader
   MR errors                           MBP readSeparator OPTIONS
  MBP foldHeaderLine LINE, LENGTH       MR report [LEVEL]
  MBP inDosmode                         MR reportAll [LEVEL]
   MR log [LEVEL [,STRINGS]]           MBP setPosition WHERE
  MBP new [OPTIONS]                    MBP start OPTIONS
   MR new OPTIONS                      MBP stop
...
static int set_position(SV *obj, long where)
{   Mailbox *box = GETPTR(obj);
    return fseek(box->file, where, 0)==0;
}
