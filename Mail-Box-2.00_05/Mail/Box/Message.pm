...
The general methods for C<Mail::Box::Message> objects:
   ...
   MM clone                                modified [BOOL]
      coerce FOLDER, MESSAGE [,OP...       new ARGS
      copyTo FOLDER                     MM new OPTIONS
      delete                            MR new OPTIONS
      deleted [BOOL]                    MM parent
      diskDelete                        MM print [FILEHANDLE]
   MR errors                            MR report [LEVEL]
      folder [FOLDER]                   MR reportAll [LEVEL]
   MM guessTimestamp                       seqnr [INTEGER]
   MM head [OBJECT]                        setLabel LIST
   MM isDelayed                            shortString
   MM isDummy                              size
   MM isMultipart                       MM timestamp
   MM isPart                            MM toplevel
      label STRING [ ,STRING ,...]      MR trace [LEVEL]
      labels                            MR warnings
