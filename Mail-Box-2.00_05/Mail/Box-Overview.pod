...
objects, each containing one header line.

The header object has four sub-classes: the C<::Complete> version knows
all lines for sure, the C<::Partial> keeps a specific subset of the lines,
the C<::Subset> maintains an unknown subset of lines, and the C<::Delayed>
has no lines yet.

The latter three will automatically get the missing header lines from the
