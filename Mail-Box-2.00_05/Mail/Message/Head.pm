...
The general methods for C<Mail::Message::Head> objects:
      ...
      add NAME, BODY [,COMMENT]         MR log [LEVEL [,STRINGS]]
      clone [FIELDS]                       message [MESSAGE]
      count NAME                           names
      createFromLine                       new OPTIONS
      createMessageId                   MR new OPTIONS
   MR errors                               print FILE [,LINE-LENGTH]
      get NAME [,INDEX]                 MR report [LEVEL]
      grepNames [NAMES|ARRAY-OF-N...    MR reportAll [LEVEL]
      guessBodySize                        reset NAME, FIELDS
      guessTimestamp                       set NAME, BODY [,COMMENT]
      isDelayed                            timestamp
