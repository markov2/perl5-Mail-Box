...
static int MBPC_set_position(SV *obj, long where)
{   Mailbox *box = GETPTR(obj);
...
{   Mailbox *box      = GETPTR(obj);
    ...
    if(expect_chars >=0)
    {
        ...
        {   /*  Accept new end  */
            ...
            Inline_Stack_Push(sv_2mortal(newSViv(begin)));
            Inline_Stack_Push(sv_2mortal(newSViv(expect_chars)));
            Inline_Stack_Push(sv_2mortal(newSViv(expect_lines)));
    ...
    if(scan_stripped_lines(box, expect_chars, expect_lines,
        &nr_chars, &nr_lines))
    {   Inline_Stack_Push(sv_2mortal(newSViv(begin)));
        Inline_Stack_Push(sv_2mortal(newSViv(nr_chars)));
        Inline_Stack_Push(sv_2mortal(newSViv(nr_lines)));
        Inline_Stack_Return(3);
    }
