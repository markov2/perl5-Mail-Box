...
The general methods for C<Mail::Box::Message> objects:
   ...
   MM attach MESSAGES [,OPTIONS]           labels
   MM body [OBJECT]                     MR log [LEVEL [,STRINGS]]
   MM clone                             MM messageId
      coerce FOLDER, MESSAGE [,OP...       modified [BOOL]
      copyTo FOLDER                        new OPTIONS
      delete                            MM parent
      deleted [BOOL]                    MM print [FILEHANDLE]
      diskDelete                        MR report [LEVEL]
   MR errors                            MR reportAll [LEVEL]
      folder [FOLDER]                      seqnr [INTEGER]
   MM guessTimestamp                       setLabel LIST
   MM head [OBJECT]                        shortString
   MM isDelayed                            size
   MM isDummy                           MM timestamp
   MM isMultipart                       MM toplevel
   MM isPart                            MR trace [LEVEL]
      label STRING [ ,STRING ,...]      MR warnings
