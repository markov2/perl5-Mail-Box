...
The general methods for C<Mail::Box> objects:
      ...
      DESTROY                           MR log [LEVEL [,STRINGS]]
      activeMessage INDEX [,MESSAGE]       message INDEX [,MESSAGE]
      addMessage  MESSAGE                  messageDeleted MESSAGE|MESS...
      ...
      current [NUMBER|MESSAGE|MES...       registerHeaders REGEXP [,RE...
      delete                               registeredHeaders
   MR errors                            MR report [LEVEL]
      find MESSAGE-ID                   MR reportAll [LEVEL]
      folderdir [DIR]                   MR trace [LEVEL]
      foundIn FOLDERNAME, OPTIONS       MR warnings
      lazyExtract HEADER, BODY, SIZE       write OPTIONS
      listFolders OPTIONS                  writeable
...
implementation of C<Mail::Box::Mbox>.  As normal users of folders
...
of a code, the header-structure is passed as only argument to the routine.
